...
const FilterSection = ({ 
  ...
  const productTypeIcons: Record<string, JSX.Element> = {
    'stress-management': <Zap className="w-4 h-4" />,
    'mycorrhiza': <Sprout className="w-4 h-4" />,
    'organic-carbon': <Leaf className="w-4 h-4" />,
    'special-application': <Target className="w-4 h-4" />,
  ...
  const productTypeLabels: Record<string, string> = {
    ...
    'special-application': "Special Applications",
        ...
        <div className="relative h-48 overflow-hidden bg-gradient-to-br from-green-50 to-white">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1">
            ...
            <Badge className={`${productTypeColors[productType]} text-white text-xs font-semibold`}>
              {productType.replace('-', ' ').charAt(0).toUpperCase() + productType.replace('-', ' ').slice(1)}
            </Badge>
          </div>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div>
                ...
                <div className="flex gap-2 mb-4">
                  ...
                  <Badge className={`${
                    productType === 'stress-management' ? 'bg-green-500' :
                    productType === 'mycorrhiza' ? 'bg-green-600' :
                    productType === 'organic-carbon' ? 'bg-green-700' :
                    'bg-green-500'
                  } text-white`}>
                    {productType.replace('-', ' ').charAt(0).toUpperCase() + productType.replace('-', ' ').slice(1)}
                  </Badge>
